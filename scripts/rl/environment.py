...
class SwarmRLWrapper:
    ...
    def __init__(self, cfg):
        ...
        self.sub_steps = int(cfg["sim"]["substeps"]["live"]) # Use live steps for training speed
        
        # Override config for RL if needed
        # We want the RL agent to control the acceleration directly
        self.a_max = float(cfg["controller"]["dim_defaults" if self.dim==2 else "dim_defaults"]["a_max_2d" if self.dim==2 else "a_max_3d"])
    ...
    def step(self, action_rl):
        ...
        self.ctrl.constraint_builder.add_constraints(a["pos"], a["vel"], threats, self.dim==2)
        ...
        class RLOverrideController:
            ...
            def compute_control(self, x, v, goal, obstacles, i, all_agents):
                if i == 0:
                    return self.u_0
                return self.real_ctrl.compute_control(x, v, goal, obstacles, i, all_agents)
        ...
        reward, info = self._compute_reward(
            self.agents[0], u_rl, u_safe, delta
        )
